...
const fileContentsTemplate = `.${COMPONENT_NAME_PLACEHOLDER} {
}
`;
...
export function createCssFile(targetDir: string, componentName: string) {
  ...
  const fileContents = fileContentsTemplate.replaceAll(COMPONENT_NAME_PLACEHOLDER, lowercaseFirstChar(componentName));
